...
export default function useEmbryoInoutGroupList() {
    const {page, pageSize, in_out} = useParameters()
    let key = `${host}/api/embryo/inout/group/list?page=${page}&page_size=${pageSize}`
    if (in_out!= undefined) {
        key = `${host}/api/embryo/inout/group/list?page=${page}&page_size=${pageSize}&in_out=in_out`
    }
